...
!         REAL(KIND=8) :: Vol,A,Ev,Ea,KV,KA 
!         REAL(KIND=8) :: A_initial,V_initial
         energy=0.0
...
! Sanjoy edits end here
         ...
         energy=energy+((ver(v)%mcur-ver(v)%cpar)**2)*kappa*ver(v)%totarea+spenergy+necuen                        !  Helfrich+Nem-nem+Nem-curvature       
         ENDDO flip_vertices
...
! Sanjoy edits end here
         ...
         Call Nematic_Interaction_Potential(spenergy,v,v1)                                    ! Nematic - Nematic interaction with all neigh spins   
	 	 Call Nematic_Curvature_Potential(necuen,v)
         energy=energy+((ver(v)%mcur-ver(v)%cpar)**2)*kappa*ver(v)%totarea+spenergy+necuen-pr*tri(tn1)%vol          !  Helfrich favouring a positive spontaneous curv       
         ENDDO mv_neigh 
         necuen=0.0 ; Call Nematic_Curvature_Potential(necuen,vert)
         energy=energy+((ver(vert)%mcur-ver(v)%cpar)**2)*kappa*ver(vert)%totarea+necuen                             !  Helfrich +Nematic-curvature for the displaced vertex 
...
!------------------------------------------------------------------------------------------------------------------------
        ...
        helfenergy = 0.0
		helfenergy = kappa*ver(v)%totarea*((ver(v)%mcur-ver(v)%cpar)**2)
