...
!     Restricting link flip across the two phases (SANJOY edits)
        IF((ver(vt1)%phase .ne. 3) .And. (ver(vt2)%phase .ne. 3) .And. &
        (ver(lp1)%phase .ne. 3) .And. (ver(lp2)%phase .ne. 3)) THEN
        IF ((ver(vt1)%phase .ne. ver(vt2)%phase) .OR. (ver(lp1)%phase & 
       .ne. ver(lp2)%phase)) RETURN             
        ENDIF
...
!Sanjoy edits
         Ev = KV*((Vol1/V_initial)-1)**2
         Ea = KA*((Area1/A_initial)-1)**2
! Sanjoy edits end here
                      
        IF(((fien-inen+Ev+Ea).GT.0).And.(ran2(seed).Gt.exp(-beta*(fien-inen+Ev+Ea))))  mchk=1                                                   ! pass the control of the two faces are perpn 
...
!---------------------------------------------------------------------------------------------------------------------------
        ...
        REAL(KIND=8),DIMENSION(3,1)::dr,vcoord1                                                                                             ! Values of the Nematic Vector            
...
! Sanjoy edits end here
        ...
        mvmetropolis:IF(((fien-inen+Ev+Ea).GT.0.0).And.(ran2(seed).Gt.exp(-beta*(fien-inen+Ev+Ea)))) THEN
...
!---------------------------------------------------------------------------------------------------------------------------
           ...
           if ((phase1 .Eq. 2) .Or. (phase2 .Eq. 2)) Return
